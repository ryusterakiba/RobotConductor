...
text_file = open(output_folder + "/Output.txt", "w")
try:
    text_file.write("Input Folder: %s" % input_folder)
    text_file.write("Output Folder: %s" % output_folder)
    text_file.write("Date: %s" % datetime.datetime.now())
finally:
    text_file.close()
...
def main():
    ...
    for _, fn in enumerate(list_of_images):
        ...
        f = open(output_folder + "/" + file_prefix + ".txt", "w")
        ...
        try:
            ...
            print(input_folder + "-" + fn + " has been failed !!")
